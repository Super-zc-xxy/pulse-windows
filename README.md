...
构建产物位于 `src-tauri/target/release/bundle/`；显式指定目标时位于 `src-tauri/target/<target>/release/bundle/`。每个平台在本平台构建；`.github/workflows/desktop.yml` 定义了 macOS、Windows、Ubuntu 构建矩阵，尚未执行。签名、公证、发布均未配置或执行。
